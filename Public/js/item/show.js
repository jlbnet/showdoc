...
$(function () {
  ...
  if ((current_page_id != null && current_page_id.toString().length > 0) ||
    ...
    $(".doc-left li").each(function () {
      ...
      if ((page_id != null && page_id.toString().length > 0 && page_id == current_page_id) ||
        ...
        if (file_id != undefined && file_id != '' && file_id != '#') {
  ...
  $(".doc-left li").click(function () {
    //先把所有菜单的激活状态取消
    $(".doc-left li").each(function (i, value) {
      $(value).removeClass("active");
    });
    //先判断是否存在子菜单
    if ($(this).children('.child-ul').length != 0) {
      //如果子菜单是隐藏的，则显示之；如果是显示状态的，则隐藏
      console.log($(this).children('.child-ul').css("display"));
  ...
  $('.watch-btn').click(function () {
    ...
    var url = TYPE == 'page' ? '/Home/Page/watch' : '/Home/File/watch';

    $.ajax({
      type: 'post',
      url: base_url + url,
      data: JSON.stringify({
        id: $("#current_page_id").val(),
        state: watch
      }),
  ...
  function getWatchData(type, page_id) {
    var url = type == 'page' ? '/Home/Page/getWatchData' : '/Home/File/getWatchData';
    $.ajax({
      type: 'post',
      url: base_url + url,
      data: JSON.stringify({
      ...
      success: function (res) {
        if (res.errno == '200') {
          changeWatch(res.data.isWatched);
          rerenderWatchList(res.data.users);
        } else {
          console.log(res.message);
        }
      }
